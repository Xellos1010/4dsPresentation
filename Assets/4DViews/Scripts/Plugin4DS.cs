...
#region Properties
        ...
        public string[] GetSequenceNames()
        {
            ...
            string[] directories = Directory.GetFiles(directoryPath, "*.4ds");
            SequenceNames = new string[directories.Length];
            for(int i = 0; i < directories.Length; i++)
            {
                SequenceNames[i] = new DirectoryInfo(directories[i]).Name;
            }
            Debug.Log(SequenceNames.Length);
            return SequenceNames;
        }
        public void NextSequence()
        {
            currentSequence+=1;
            if (currentSequence>= SequenceNames.Length)
                currentSequence = 0;
            SequenceName = SequenceNames[currentSequence];
        }
